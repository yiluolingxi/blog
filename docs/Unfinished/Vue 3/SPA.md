SPA（Single Page Application），我们通常指的是单页面应用程序，顾名思义只有一个HTML页面，通过 Ajax 和动态 HTML5（DOM）来实现，无需重新加载整个页面就能响应用户操作。与传统的 MPA 相比，响应更快、用户体验更流畅，因为用户每次与应用程序交互时不需要等待整个页面重新加载，这样减少服务器请求次数，同时也能避免页面重载卡顿。
...
总之，Ajax 技术是 SPA 实现的关键之一，它可以帮助 SPA 实现异步请求数据、前端路由、部分更新等功能，从而提高了应用程序的性能和用户体验
